...
2.  Compile the source files using the C++14 standard:
    ```bash
    g++ -std=c++14 main.cpp cpu/*.cpp parser/*.cpp -w -o cpu_sim
    ```
...
The compiled simulator (`./cpu_sim`) is executed via the command line, requiring an input assembly file and configuration flags for logging and the chosen branch prediction algorithm.

#### Syntax

```bash
./cpu_sim <input.s> [OPTIONS]
```
...
| Command | Description |
| :--- | :--- |
| `./cpu_sim benchmark_1.s --GAg --log cout` | Analyze `benchmark_1.s` using the GAg method, outputting results to the console. |
| `./cpu_sim test_loop.s --PAg --log pag_run_metrics.txt` | Simulate `test_loop.s` using the PAg method and save the detailed metrics to a file. |
